...
function __kubeswitch_util_message --description="Prints a message"
	...
	# Print the message.
	printf "%s%s%s\n" "$active_color_code" (printf $__promptfessional_printf_accept_pattern_variable $argv) "$reset_color_code"
	return 0
end
...
function __kubeswitch_real_kubectl --description="Runs the real kubectl command"
	if [ -n "$kubeswitch_kubectl" ]
		...
		return $status
	elif [ -n "$__kubeswitch_real_kubectl_command" ]
	...
	if grep -F -- "Generated by fish-kubeswitch" "$kubectls[1]" &>/dev/null
		set -e kubectls[1]
	end
